...
function Recurrent:forward(input)
   ...
   copy(self.cellState[0], input[self.N+2])
   for i = 1,self.N do
      local stepInput  = {self.hidden[i-1], self.cellState[i-1], input[i]}
      --print(stepInput)
      local stepOutput = self.step:forward(stepInput)
      copy(self.hidden[i], stepOutput[1])
      copy(self.cellState[i], stepOutput[2])
      copy(self.output[i], stepOutput[3])
   end
   return self.output
end
...
function Recurrent:backward(input, gradOutput)
   ...
   zero(self.gradInput[self.N+2], input[self.N+2])
   for i = self.N,1,-1 do
      local stepInput  = {self.hidden[i-1], self.cellState[i-1], input[i]}
      local stepOutput = self.step:forward(stepInput)
      local stepGradOutput = {self.gradInput[self.N+1], self.gradInput[self.N+2], gradOutput[i]}
      local stepGradInput  = self.step:backward(stepInput, stepGradOutput)
      copy(self.gradInput[self.N+1], stepGradInput[1])
      copy(self.gradInput[self.N+2], stepGradInput[2])
      copy(self.gradInput[i], stepGradInput[3])
   end
